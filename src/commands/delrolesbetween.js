...
const command = {
  name: 'delrolesbetween',
  category: 'Roles',
  description: 'Delete every deletable role strictly between two given roles',
  usage: 'delrolesbetween <role1> <role2>',
  example: ',delrolesbetween @Top @Bottom',
  ...
  async execute(message, args) {
    ...
    const roleA = resolveRole(message.guild, args[0]);
    const roleB = resolveRole(message.guild, args[1]);
    if (!roleA || !roleB) {
      return message.reply({ embeds: [buildUsageEmbed(command)] });
    }

    if (roleA.id === roleB.id) {
      return message.reply({ embeds: [failEmbed(message.author.id, 'Those are the same role')] });
    }

    await message.guild.roles.fetch();

    const [lower, upper] = roleA.position < roleB.position ? [roleA, roleB] : [roleB, roleA];

    const toDelete = [...message.guild.roles.cache.values()].filter(
      (role) => role.position > lower.position && role.position < upper.position && role.editable,
    );

    if (toDelete.length === 0) {
      return message.reply({ embeds: [plainEmbed(`No deletable roles strictly between **${lower.name}** and **${upper.name}**`)] });
    }

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('delrolesbetween_confirm').setLabel('Delete all').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId('delrolesbetween_cancel').setLabel('Cancel').setStyle(ButtonStyle.Secondary),
    );
    ...
    const response = await message.reply({
      embeds: [
        warnEmbed(
          message.author.id,
          `Delete **${toDelete.length}** role(s) between **${lower.name}** and **${upper.name}**? This cannot be undone`,
        ),
      ],
      components: [row],
    });
    ...
    collector.on('collect', async (interaction) => {
      ...
      if (interaction.customId === 'delrolesbetween_cancel') {
      ...
      for (const role of toDelete) {
        try {
          await role.delete('tomichu ,delrolesbetween');
          success++;
        } catch {
          failed++;
        }
